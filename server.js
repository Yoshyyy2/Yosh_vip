...
function buildRemoteScript(remoteHost, remotePort, inputs) {
  let script = `set timeout 90
spawn ssh -tt -i ${SSH_KEY} -p ${remotePort} -o StrictHostKeyChecking=no root@${remoteHost}
`;
  script += `expect -re {[#$] $}
`;
  script += `send "menu\r"
...
function runRemoteExpect(remoteHost, remotePort, scriptContent) {
  return new Promise((resolve, reject) => {
    const id = Date.now();
    const tmp = `/tmp/vipweb_${id}.exp`;
    fs.writeFileSync(tmp, scriptContent);
...
async function createSSHRemote(remoteHost, remotePort, username, password, days) {
  const script = buildRemoteScript(remoteHost, remotePort, [
    ['Option:', '1'],
    ['Option:', '1'],
