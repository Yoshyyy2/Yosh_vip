...
function runRemoteExpect(remoteHost, remotePort, script) {
  return new Promise((resolve, reject) => {
    const id = Date.now();
    const localTmp = `/tmp/vipweb_${id}.exp`;
    const remoteTmp = `/tmp/vipweb_${id}.exp`;

    // Build the expect script that runs locally on the remote server
    const expScript = `set timeout 120
spawn menu
${script.trim()}`;

    // Write expect script locally, scp it to remote, run it there, then delete it
    fs.writeFileSync(localTmp, expScript);

    const cmd = [
      `scp -i ${SSH_KEY} -P ${remotePort} -o StrictHostKeyChecking=no "${localTmp}" root@${remoteHost}:"${remoteTmp}"`,
      `ssh -i ${SSH_KEY} -p ${remotePort} -o StrictHostKeyChecking=no root@${remoteHost} "expect \"${remoteTmp}\" && rm -f \"${remoteTmp}\""`,
    ].join(' && ');
    ...
    exec(cmd, { timeout: 120000 }, (err, stdout, stderr) => {
      ...
      console.log('[REMOTE OUTPUT]', output.slice(-800));
      if (err && !output) return reject('Timeout/Error: ' + (err.message || ''));
      resolve(output);
    });
  });
}
...
async function createSSHRemote(remoteHost, remotePort, username, password, days) {
  ...
  const out = await runRemoteExpect(remoteHost, remotePort, lines.join('\n'));
...
async function createVLESSRemote(remoteHost, remotePort, username, days, sni) {
  const lines = [
    ...
    'expect "Option:"', 'send "1\r"',
    'expect "Option:"', 'send "1\r"',
    'expect "Enter username:"', `send "${username}\r"`,
    'expect "Expiration (days):"', `send "${days}\r"`,
    'expect "Enter SNI :"', `send "${sni}\r"`,
    'expect "Press Enter to continue..."', 'send "\r"',
    'expect "Option:"', 'send "0\r"',
    'expect "Option:"', 'send "0\r"',
    'expect "Option:"', 'send "0\r"',
    'expect eof'
  ];
...
const REMOTE_CONFIGS = {
  "jp": {
    ...
    "SERVER_PUBKEY": "cd879d3e6dda1acae7f30450cd58e3c0bc385d18a5497ed11f766805e3c4d52f"
  },
  "es": {
...
// GET /api/ping/:srv - TCP check remote servers
app.get('/api/ping/:srv', (req, res) => {
  const { srv } = req.params;
  ...
  socket.connect(srvInfo.port, srvInfo.host, () => {
    const ms = Date.now() - start;
    socket.destroy();
    res.json({ ok: true, ms });
  });
  socket.on('error', () => { socket.destroy(); res.status(503).json({ ok: false }); });
  socket.on('timeout', () => { socket.destroy(); res.status(503).json({ ok: false }); });
});
