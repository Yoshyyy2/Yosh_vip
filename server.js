...
spawn ssh -i ${SSH_KEY} -p ${remotePort} -o StrictHostKeyChecking=no -o ConnectTimeout=10 root@${remoteHost} menu
proc s {txt} { sleep 0.3; send -- "$txt\r" }
${script.trim()}`;
    fs.writeFileSync(tmp, sshScript);
    console.log('[REMOTE EXPECT SCRIPT]', sshScript);
    exec(`expect "${tmp}"`, { timeout: 120000 }, (err, stdout, stderr) => {
      try { fs.unlinkSync(tmp); } catch (_) {}
      const output = stdout + stderr;
      console.log('[REMOTE EXPECT OUTPUT]', output.slice(-800));
...
async function createVLESSRemote(remoteHost, remotePort, username, days, sni) {
  const lines = [
    ...
    'expect "Option:"', 's "1"',
    'expect "Option:"', 's "1"',
    'expect "Enter username:"', `s "${username}"`,
    'expect "Expiration (days):"', `s "${days}"`,
    'expect "Enter SNI :"', `s "${sni}"`,
    'expect "Press Enter to continue..."', 's ""',
    'expect "Option:"', 's "0"',
    'expect "Option:"', 's "0"',
    'expect "Option:"', 's "0"',
    'expect eof'
...
async function createTrojanRemote(remoteHost, remotePort, password, days, sni) {
  const lines = [
    ...
    'expect "Option:"', 's "1"',
    'expect "Option:"', 's "3"',
    'expect "Enter password:"', `s "${password}"`,
    'expect "Expiration (days):"', `s "${days}"`,
    'expect "Enter SNI :"', `s "${sni}"`,
    'expect "Press Enter to continue..."', 's ""',
    'expect "Option:"', 's "0"',
