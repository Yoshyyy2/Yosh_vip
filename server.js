...
function buildRemoteScript(inputs) {
  // inputs = array of [expect_pattern, send_value] pairs
  // Returns a proper expect script string
  let script = 'set timeout 90\nspawn menu\n';
  for (const [pattern, value] of inputs) {
    script += `expect "${pattern}"\n`;
    script += `send "${value}\r"\n`;
  }
  script += 'expect eof\n';
  return script;
...
function runRemoteExpect(remoteHost, remotePort, scriptContent) {
  return new Promise((resolve, reject) => {
    const id = Date.now();
    const localTmp = `/tmp/vipweb_${id}.exp`;
    const remoteTmp = `/tmp/vipweb_${id}.exp`;

    fs.writeFileSync(localTmp, scriptContent);

    const scp = `scp -i ${SSH_KEY} -P ${remotePort} -o StrictHostKeyChecking=no -o ConnectTimeout=10 "${localTmp}" root@${remoteHost}:${remoteTmp}`;
    const run = `ssh -i ${SSH_KEY} -p ${remotePort} -o StrictHostKeyChecking=no root@${remoteHost} "expect ${remoteTmp}; rm -f ${remoteTmp}"`;

    exec(`${scp} && ${run}`, { timeout: 120000 }, (err, stdout, stderr) => {
      try { fs.unlinkSync(localTmp); } catch (_) {}
      const output = stdout + stderr;
      console.log('[REMOTE OUTPUT]', output.slice(-1000));
      if (err && !output) return reject('Error: ' + (err.message || ''));
      resolve(output);
    });
  });
}
