...
app.post('/api/login', (req, res) => {
  ...
  if (entry.inUse) return res.status(401).json({ error: 'Wrong password!' });
...
app.post('/api/create/vless', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const expiry = expiryISO(expDays);
    const db = loadDB();
    db.accounts.push({ id: Date.now().toString(), username, type: 'vless', expiry, ip: getIP(req), createdAt: new Date().toISOString(), tls, nonTls });
    saveDB(db);
...
app.post('/api/create/vmess', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const expiry = expiryISO(expDays);
    const db = loadDB();
    db.accounts.push({ id: Date.now().toString(), username, type: 'vmess', expiry, ip: getIP(req), createdAt: new Date().toISOString(), tls, nonTls });
    saveDB(db);
